...
class PrisonerPatientUpdateService(
  ...
  fun externalMovement(externalMovement: ExternalPrisonerMovementMessage) {
    ...
    if (externalMovement.movementType in listOf("ADM", "REL")) {
      ...
      val establishmentCode =
        if (changeType == REGISTRATION) externalMovement.toAgencyLocationId else externalMovement.fromAgencyLocationId
  ...
  private fun updateNhsSystem(offender: PrisonerStatus, changeType: ChangeType, establishmentCode: String): Boolean {
    ...
    offenderPatientRecordRepository.save(
      OffenderPatientRecord(
        offender.nomsId,
        gson.toJson(offender),
        LocalDateTime.now()
      )
    )
    ...
    prisonRegisterService.getPrisonRegisterByPrisonId(establishmentCode)?.let { prison ->
      ...
      with(offender) {
        val nhsPrisoner = NhsPrisoner(
          nomsId, prison.gpPracticeCode, establishmentCode, givenName1, givenName2, lastName,
